...
export async function deleteTask(id: string){
  try {
    const deleteAndUpdateTask = await prisma.$transaction(async (tx) => {
      const deletedTask = await tx.task.delete({
        where: { id },
      });

      if (deletedTask.position > 0) {
        const updatedTask = await tx.task.updateManyAndReturn({
          where: {
      ...
      return { title: deletedTask.title };
    });
    if(deleteAndUpdateTask.updatedTask){
      return {
        success: true,
        data: deleteAndUpdateTask.updatedTask as Task[] ,
        message: `${deleteAndUpdateTask.title} task has been deleted successfully`,
      };
    }
    return {
      success: true,
